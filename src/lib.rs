use options::{FileStats, Handlers, Options, OutputOption};
use rayon::prelude::*;
use std::sync::Arc;
use std::sync::Mutex;
...
pub fn run<'a, T: Sync>(
    ...
    let recurse = if let OutputOption::All = opts.output {
        true
    } else {
        false
    };
...
fn check_path<'a, T: Sync>(
                    ...
                    *mut_total += size;
                    if output {
                        if let Some(handle) = &handlers.prog {
                            ...
                            update_stats(&mut stats_copy, &start, mut_total.clone(), true);
    ...
    if output {
        update_stats(&mut stats, &start, size, success);
...
fn update_stats(stats: &mut FileStats, start: &Instant, size: u64, success: bool) {
